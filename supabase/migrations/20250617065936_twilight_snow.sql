/*
  ...
  2. Create new functions with correct return types
  3. Grant proper permissions
  
  This migration adds the missing RPC functions that the frontend needs:
  - hall_ticket_exists: Check if hall ticket is already registered
  - email_exists: Check if email is already registered  
  - get_complete_user_profile: Get complete user profile with relations
  - create_complete_user_profile: Create new user with all related records
*/

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS public.get_complete_user_profile(text);
DROP FUNCTION IF EXISTS public.create_complete_user_profile(text, text, text, text, text, text, text, boolean);
DROP FUNCTION IF EXISTS public.hall_ticket_exists(text);
...
GRANT EXECUTE ON FUNCTION public.hall_ticket_exists(text) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.email_exists(text) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.get_complete_user_profile(text) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.create_complete_user_profile(text, text, text, text, text, text, text, boolean) TO authenticated, anon;